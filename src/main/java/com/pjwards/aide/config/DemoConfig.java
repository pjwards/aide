...
public class DemoConfig {
    ...
    public CommandLineRunner conferenceDemo(ConferenceService conferenceService,
        ...
        return (args) -> {
            ...
            Conference conference = conferenceService.add(new Conference.Builder("DEVIEW 2015", "DEVIEW 2015가 성황리에 끝났습니다.",
            ...
            conferenceService.add(new Conference.Builder("PYCON KOREA 2014", "한국에서 열리는 첫 번째 파이콘", "한국에서 열리는 첫 번째 파이콘").host(user).build());

            Set<Contact> contacts = new HashSet<>();
            contacts.add(contactService.add(new Contact.Builder(ContactType.EMAIL, "abcde@abcde.com").conference(conference).build()));
            contacts.add(contactService.add(new Contact.Builder(ContactType.FACEBOOK, "http://www.facebook.com").conference(conference).build()));
            contacts.add(contactService.add(new Contact.Builder(ContactType.TWITTER, "www.twitter.com").conference(conference).build()));

            Set<User> managers = new HashSet<>();
            managers.add(user3);
            Room room1 = roomService.add(new Room.Builder("100호", "100", "100호").conference(conference).build());
            room1.setManagerSet(managers);
            roomService.update(room1);

            Room room2 = roomService.add(new Room.Builder("101호", "101", "101호").conference(conference).build());
            Room room3 = roomService.add(new Room.Builder("102호", "102", "102호").conference(conference).build());

            messageService.add(new Message.Builder("Test message 1", user, room1).build());
            messageService.add(new Message.Builder("Test message 2", user, room1).build());
            messageService.add(new Message.Builder("Test message 3", user, room1).build());
            messageService.add(new Message.Builder("Test message 4", user, room1).build());

            // Day 1
            ProgramDate programDate1 = programDateService.add(
                    new ProgramDate.Builder("Day 1", "2016-01-01").conference(conference).build());
            programService.add(new Program.Builder("참가등록", "참가등록", "08:40", "09:20")
                    .date(programDate1)
                    .room(room1)
                    .speakerSet(userSet)
                    .programType(ProgramType.REGISTER).build());

            programService.add(new Program.Builder("키노트", "키노트", "09:20", "09:40")
            ...
            Program program1 = programService.add(new Program.Builder("세션1", "테스트", "10:00", "10:50")
                    .date(programDate1)
                    .speakerSet(userSet)
                    .programType(ProgramType.SESSION).build());

            Program program2 = programService.add(new Program.Builder("세션2", "테스트", "11:00", "11:50")
                    .date(programDate1)
                    .speakerSet(userSet)
                    .programType(ProgramType.SESSION).build());

            sessionService.add(new Session.Builder("강좌1", "강좌 설명")
                    .videoUrl("www.google.com")
            ...
            sessionService.add(new Session.Builder("강좌2", "테스트2")
                    .videoUrl("www.google.com")
                    .program(program1)
                    .room(room2)
                    .speakerSet(userSet).build());

            sessionService.add(new Session.Builder("강좌3", "테스트3")
            ...
            sessionService.add(new Session.Builder("강좌4", "테스트4")
                    .program(program2)
                    .room(room3)
                    .speakerSet(userSet).build());

            programService.add(new Program.Builder("점심시간", "점심시간", "12:00", "12:50")
                    .date(programDate1)
                    .room(room1)
                    .speakerSet(userSet)
            ...
            Program program3 = programService.add(new Program.Builder("세션1", "테스트", "10:00", "10:50")
                    .date(programDate2)
                    .speakerSet(userSet)
                    .programType(ProgramType.SESSION).build());

            Program program4 = programService.add(new Program.Builder("세션2", "테스트", "11:00", "11:50")
                    .date(programDate2)
                    .speakerSet(userSet)
                    .programType(ProgramType.SESSION).build());

            sessionService.add(new Session.Builder("강좌1", "테스트1")
                    .program(program3)
                    .room(room2)
                    .speakerSet(userSet).build());
